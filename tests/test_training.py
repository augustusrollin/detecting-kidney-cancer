...
def test_model_training(mock_data):
    ...
    model_manager.train(X_train, y_train, epochs=1500, batch_size=32)
...
def test_model_prediction(mock_data):
    ...
    model_manager.train(X, rng.integers(0, OUTPUT_SIZE, size=len(X)), epochs=1500, batch_size=32)
...
def test_mismatched_data_labels():
    ...
    with pytest.raises(ValueError, match="Size mismatch between features and labels"):
        model_manager.train(X, y, epochs=1500)
...
def test_model_save_load(mock_data, tmp_path):
    ...
    # Train the model
    model_manager.train(X, y, epochs=1500, batch_size=32)
